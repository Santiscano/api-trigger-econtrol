...
class ClearhistryModel {
  ...
  getTBpedidosPagadosModel = async (preliquidacion) => {
    const [query] = await connect.query(
      "SELECT PRE_LIQUIDACION FROM TB_PRE_LIQUIDACION_TMP_HISTORICO WHERE PRE_LIQUIDACION  IN (?)",
      [preliquidacion]
    );
  ...
  updateEmptyAuthPagadosMomModel = async (preliquidacion) => {
    const [query] = await connection.query(
      'UPDATE TB_NUMERACIONES_COMPROBANTES_GENERADOS  SET HISTORICOS = "SIN PEDIDOS"  WHERE CONSECUTIVO IN (?)',
      [preliquidacion]
    );
  ...
  getAllPedidosPagadosMadreModel = async (preliquidacion) => {
    const [query] = await connection.query(
      "SELECT * FROM TB_PRE_LIQUIDACION_TMP_HISTORICO WHERE PRE_LIQUIDACION IN (?)",
      [preliquidacion]
    );
  ...
  InsertEspejo = async (datos) => {
    const [query] = await connect.query(
      `INSERT IGNORE INTO TB_PRE_LIQUIDACION_TMP_HISTORICO 
  ...
  deletePedidosPagadosMadreModel = async (preliquidacion) => {
    const [query] = await connection.query(
      "DELETE FROM TB_PRE_LIQUIDACION_TMP_HISTORICO WHERE PRE_LIQUIDACION IN (?)",
      [preliquidacion]
    );
  ...
  updateAuthPagadosMomModel = async (preliquidacion) => {
    const [query] = await connection.query(
      "UPDATE TB_NUMERACIONES_COMPROBANTES_GENERADOS  SET HISTORICOS = 2  WHERE HISTORICOS = 1 AND CONSECUTIVO IN (?)",
      [preliquidacion]
    );
...
export default ClearhistryModel;
